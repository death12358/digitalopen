...
// RoundNext - 取得下一回合單
func (s *Scriptor) RoundNext(workspace, gameCode, roundID, userID string) (*RoundRecords, error) {
    res, err := s.Cache.ExecSha(_round_next, []string{workspace, gameCode}, roundID, userID)
    if err != nil {
        return nil, err
    }
...
var (
    _round_next          = "round.next"
    _round_next_Template = `
if #KEYS < 2 then
